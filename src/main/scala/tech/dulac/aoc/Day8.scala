...
object Day8 extends App {
  ...
  final case class EncryptionKey(
    ...
    def decode(code: String) = code.length match {
      case 2 => 1
      case 3 => 7
      case 4 => 4
      case 5 => if (two.contains(code.sorted)) 2 else if (three.contains(code.sorted)) 3 else 5
      case 6 => if (zero.contains(code.sorted)) 0 else if (six.contains(code.sorted)) 6 else 9
      case 7 => 8
    }

    private def zero = (for {
      t  <- top
      tl <- topLeft if tl != t
      tr <- topRight if tr != tl && tr != t
      bl <- bottomLeft if bl != tr && bl != tl && bl != t
      br <- bottomRight if br != bl && br != tr && br != tl && br != t
      b  <- bottom if b != br && b != bl && b != tr && b != tl && b != t
    } yield s"$t$tl$tr$bl$br$b").map(_.sorted).distinct

    private def two = (for {
      t  <- top
      tr <- topRight if tr != t
      m  <- middle if m != tr && m != t
      bl <- bottomLeft if bl != m && bl != tr && bl != t
      b  <- bottom if b != bl && b != m && b != tr && b != t
    } yield s"$t$tr$m$bl$b").map(_.sorted).distinct

    private def three = (for {
      t  <- top
      tr <- topRight if tr != t
      m  <- middle if m != tr && m != t
      br <- bottomRight if br != m && br != tr && br != t
      b  <- bottom if b != br && b != m && b != tr && b != t
    } yield s"$t$tr$m$br$b").map(_.sorted).distinct

    private def six = (for {
      t  <- top
      tl <- topLeft if tl != t
      m  <- middle if m != tl && m != t
      bl <- bottomLeft if bl != m && bl != tl && bl != t
      br <- bottomRight if br != bl && br != m && br != tl && br != t
      b  <- bottom if b != br && b != bl && b != m && b != tl && b != t
    } yield s"$t$tl$m$bl$br$b").map(_.sorted).distinct

  }

  object EncryptionKey {

    val empty = EncryptionKey(List.empty, List.empty, List.empty, List.empty, List.empty, List.empty, List.empty)

    private def permutations(s: String) = s.permutations.toList.head.toList

    def createKey(codes: List[String]) = codes.foldLeft(empty) { case (key, code) =>
      import key._
      code.length match {
        case 2 => copy(topRight = permutations(code), bottomRight = permutations(code))
        case 3 => copy(top = permutations(code.filterNot((topRight ++ bottomRight).contains)))
        case 4 =>
          copy(
            topLeft = permutations(code.filterNot((top ++ topRight ++ bottomRight).contains)),
            middle = permutations(code.filterNot((top ++ topRight ++ bottomRight).contains))
          )
        case 7 =>
          copy(
            bottomLeft = permutations(code.filterNot((top ++ topRight ++ topLeft ++ bottomRight ++ middle).contains)),
            bottom = permutations(code.filterNot((top ++ topRight ++ topLeft ++ bottomRight ++ middle).contains))
          )
        case _ => key
      }
    }
  ...
  val codes = input.map { case s"$a $b $c $d $e $f $g $h $i $j | $k $l $m $n" =>
    (List(a, b, c, d, e, f, g, h, i, j), List(k, l, m, n))
  }

  val part2 = codes
    .map { case (codes, ws) => (codes.sortBy(_.length), ws) }
    .map { case (codes, ws) => (EncryptionKey.createKey(codes), ws) }
    .map { case (key, ws) => ws.map(key.decode).mkString.toLong }
    .sum

  val part1 = codes.map { case (_, ws) => ws.length }.count(l => l < 5 || l > 6)
